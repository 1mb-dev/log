...
- 2026-05-18 (v3.17.0 engine bump + security-headers layer switch) — `v3.17.0` shipped. Bundles two same-day upstream releases: `v3.16.0` (markgo emits enforcing CSP with SHA-256 hash for its single inline FOUC script, HSTS, and Permissions-Policy denying camera/mic/geo/payment/sensors/FLoC; `X-XSS-Protection` retired; new `CSP_DISABLE` env for edge-proxy escape hatch) and `v3.17.0` (WCAG AA `--color-primary` lifts across 4 embedded-theme contexts — no log impact since `1mb.css` overrides at `:root:not([data-theme="light"])` and `#7dd3fc` on `#0a0a0a` already meets AA; `sw.js` `CACHE_VERSION` auto-injected from build version at server startup — every engine bump now auto-busts the SW cache). Layer switch: CSP / HSTS / `Permissions-Policy` dropped from `deploy/Caddyfile.example` + prod `/etc/caddy/conf.d/log.1mb.dev.caddy`; markgo serves alone. Net gain — hash-based `script-src` replaces prior Caddy `'unsafe-inline'`; `object-src 'none'` added; `img-src` widened to `'self' data: https:`. HSTS `preload` directive dropped (functionally inert — log isn't on the preload list). Single-header emission verified across `/`, `/about`, `/ama`, `/p`, `/p/run-your-own`, `/writing/<slug>`, `/feed.xml`. `verify-deploy.sh` clean; Lighthouse re-run pending.
...
- `make build`: stamp-mismatch guard. `build/markgo` must embed `MARKGO_REF` as a clean tag (`strings build/markgo | grep -Fxq "$(MARKGO_REF)"`); else refuse with a fix hint. Catches the dirty-tag trap where `MARKGO_SRC` points at a working tree one commit past the tag (e.g. on `main`), producing `vX.Y.Z-1-gSHA` stamps that contaminate `/health` and the SW cache version.
